...
MLFLOW_RUN_NAME = "XGBoost_Default" 
...
# XGBoost parameters (using suggested defaults from project plan + reasonable additions)
xgb_params = {
...
xgb_model = xgb.XGBClassifier(**xgb_params) # Changed model

print("Creating final ML pipeline...")
final_pipeline = Pipeline([('preprocessing', preprocessor), 
                         ('classifier', xgb_model)]) # Changed classifier name
...
with mlflow.start_run(run_name=MLFLOW_RUN_NAME) as run:
    ...
    mlflow.log_param("model_type", "XGBClassifier") # Changed model type
    mlflow.log_params(xgb_params) # Log XGB specific params
    ...
    # XGBoost within a pipeline might need feature names converted if ColumnTransformer drops them
    # Let's fit and see; may need adjustment if XGBoost throws error about feature names
    ...
    print("Logging the XGBoost pipeline model...") 
    # Using mlflow.sklearn.log_model works for pipelines containing XGBoost
    mlflow.sklearn.log_model(
        sk_model=final_pipeline, 
        artifact_path="sklearn-model", 
        input_example=X_train.iloc[:5],
        registered_model_name="pima-xgboost-default" # Changed registered name
    )
